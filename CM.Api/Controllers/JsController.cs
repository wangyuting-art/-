...
namespace CM.Api.Controllers
{
    ...
    {
        //服务积累
        Service<jw_js> service = new Service<jw_js>();
        // GET: api/js
        [HttpGet]
        public ActionResult<ResponseJson> Get()
        {
            ...
            if (key != "")
            {
                list = list.Where(o => o.jsname.IndexOf(key) > -1).ToList();
            }
            json.Data = list;
            return json;
        }
        ...
        // POST: api/js
        [HttpPost]
        public ActionResult<ResponseJson> Post([FromBody] Models.jw_js model)
        {
            ResponseJson json = new ResponseJson();
            //写不进报错
            ...
            {
                json.Code = "500";
                json.Msg = $"增加[角色：{model.jsname}]失败！";
            }
            return json;
        }
        ...
        // PUT: api/js/5
        [HttpPut("{id}")]
        public ActionResult<ResponseJson> Put(long id, [FromBody] Models.jw_js model)
        {
            ResponseJson json = new ResponseJson();
            bool flag = false;
